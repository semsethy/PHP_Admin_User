...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_FILES['collection_images']['name'][0])) {
        foreach ($_FILES['collection_images']['tmp_name'] as $key => $tmp_name) {
            if ($_FILES['collection_images']['size'][$key] <= 2097152) { // 2MB limit
        ...
        $collection_images = $_POST['existing_collection_image']?? [];
    }
            ...
            <div class="mb-3">
                ...
                <div id="main_image_preview" style="margin-top: 10px;">
                    <img src="<?php echo isset($product_datas['main_image_url']) && !empty($product_datas['main_image_url']) ? $product_datas['main_image_url'] : 'images/logos/placeholder2.jpg'; ?>" 
                        alt="Main Image" style="margin-top:20px; margin-left:20px; width: 80px; height: 80px; object-fit: cover; border-radius: 5px; box-shadow: 2px 2px 2px rgba(0, 0, 0, 0.3); border: 1px solid lightgray;">
                </div> 
                <input type="hidden" name="existing_main_image" value="<?php echo htmlspecialchars($product_datas['main_image_url']); ?>">
            </div>
            <div class="mb-3">
                <label for="collection_images" class="form-label">Collection Images (Max 5, 2MB each)</label>
                ...
                <div id="collection_images_preview" style="margin-top: 10px;">
                <?php
                    ...
                    if (isset($product_datas['collection_image_url']) && !empty($product_datas['collection_image_url'])) {
                        ...
                        if (is_array($existing_images)) {
                            foreach ($existing_images as $image) {
                                echo '<img src="' . htmlspecialchars($image) . '" alt="collection image" style="margin-top:20px; margin-left:20px; width: 80px; height: 80px; object-fit: cover; border-radius: 5px; box-shadow: 2px 2px 2px rgba(0, 0, 0, 0.3); border: 1px solid lightgray;">';
                            }
                        } else {
                            
                            echo "<img class='current-image' src='images/logos/placeholder2.jpg' alt='Current Category Image' style='margin-top:20px; margin-left:20px; width: 80px; height: 80px; object-fit: cover; border-radius: 5px; box-shadow: 2px 2px 2px rgba(0, 0, 0, 0.3); border: 1px solid lightgray;'>";
                        }
                    } else {
                        echo "<img class='current-image' src='images/logos/placeholder2.jpg' alt='Current Category Image' style='margin-top:20px; margin-left:20px; width: 80px; height: 80px; object-fit: cover; border-radius: 5px; box-shadow: 2px 2px 2px rgba(0, 0, 0, 0.3); border: 1px solid lightgray;'>";
                    }
                ...
                <input type="hidden" name="existing_collection_image" value="<?php echo htmlspecialchars($existing_images); ?>">
            </div>
            <div class="mb-3">
...
<script>
    function previewMainImage(event) {
        ...
        reader.onload = function() {
            ...
            output.innerHTML = '<img src="' + reader.result + '" alt="Main Image" style="width: 100px; height: 100px; object-fit: cover; border-radius: 5px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">';
        };
