<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
            ...
            // If there's an existing image, use it.
        ...
        if (!empty($title)) {
            if (isset($_POST['slideshow_id']) && $_POST['slideshow_id'] != '') {
                ...
                $stmt = $slideshow->update($title, $image_path, $caption, $description, $link, $status, $category_id, $slideshow_id);
                $action_message = "Updated successfully!";
            } else {
                $stmt = $slideshow->create($title, $image_path, $caption, $description, $link, $status, $category_id);
                ...
                <?php if (isset($slideshow)): ?>
                    ...
                    <div id="image_preview">
                        <?php if (isset($slideshow['image']) && !empty($slideshow['image'])): ?>
                            <img class="current-image" src="<?php echo htmlspecialchars($slideshow['image']); ?>" alt="Current Image" style="margin-top:20px; margin-left:20px; width: 160px; height: 80px; object-fit: cover; border-radius: 5px; box-shadow: 2px 2px 2px rgba(0, 0, 0, 0.3); border: 1px solid lightgray;">
                        <?php else: ?>
...
<script>
    function previewImage(event) {
        ...
        reader.onload = function() {
            ...
            output.innerHTML = '<img src="' + reader.result + '" alt="Selected Image" style="margin-top:20px; margin-left:20px; width: 160px; height: 80px; object-fit: cover; border-radius: 5px; box-shadow: 4px 4px 4px rgba(0, 0, 0, 0.3);">';
        };
...
<?php
    ...
    $conn = new mysqli("localhost", "root", "root", "dba3B");
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (!empty($title)) {
            ...
            if (isset($_POST['id']) && $_POST['id'] != '') {
                ...
                $stmt = $conn->prepare("UPDATE categories SET category_name = ?, status = ?, category_image = ? WHERE id = ?");
                $stmt->bind_param("sisi", $category_name, $category_status, $category_image, $category_id);
                $action_message = "updated successfully!";
            } else {
                // Insert new category
                $stmt = $conn->prepare("INSERT INTO slideshow (title, image, caption, description, link, status, category_id) VALUES (?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param("sssssii", $title, $image_path, $caption, $description, $link, $status, $category_id);
                $action_message = "New slideshow added successfully!";
            }
    
            if ($stmt->execute()) {
                echo "<div id='success-alert' style='width:80%;' class='container alert alert-success mt-4 mb-4'>$action_message</div>";
                // Redirect or close connection
                echo "<script>setTimeout(function(){ $('#success-alert').fadeOut(1000); }, 2000);</script>";
            } else {
                echo "<div id='error-alert' class='alert alert-danger'>Error: " . $stmt->error . "</div>";
                echo "<script>setTimeout(function(){ $('#error-alert').fadeOut(1000); }, 2000);</script>";
